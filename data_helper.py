...
def load_data_and_write_to_file(data_file, train_data_file, test_data_file, test_sample_percentage):
    ...
    # Write to CSV file
    with open(train_data_file, 'w', newline='', encoding='utf-8') as f:
...
def preprocess(data_file, save_dir='./data/', vocab_size=50000, padding_size=200):
    ...
    text_preprocesser = keras.preprocessing.text.Tokenizer(num_words=vocab_size)#, oov_token="<UNK>")
    text_preprocesser.fit_on_texts(corpus)
    x = text_preprocesser.texts_to_sequences(corpus)
    word_dict = text_preprocesser.word_index
    # save word2id
    with open(save_dir+'voab.txt', 'w', encoding='utf-8') as f:
